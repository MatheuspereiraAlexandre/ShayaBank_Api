...
async fn main() -> std::io::Result<()> {
    // Conecta no MongoDB
    let client_options = ClientOptions::parse("mongodb://localhost:27017/").await.unwrap(); // a url do mongo claro q tem q user .env
...
struct UserInput { // input do usuario (sem ScodeN pois vai ser gerado aqui)
    name: String, // nome 
...
async fn register(
...
) -> impl Responder {  // função para registrar
    ...
    let scode_hash = match argon2.hash_password(scode_n_plain.as_bytes(), &salt2) {
        Ok(hash) => hash.to_string(),
        Err(_) => return HttpResponse::InternalServerError().body("Erro ao criptoafsfa essa desgraça ai ScodeN"), // memsa coisa da senha
    };

    // Salva no banco os hashes password_hash e scode_hash com o nome
    let created_at = chrono::Utc::now();
    let user_doc = UserDocument { // isso daqui é a definição do document do user para o mongo
    ...
    if let Err(e) = db.insert_one(user_doc, None).await {
        return HttpResponse::InternalServerError().body(format!("Erro ao salvar na porra do banco: {}", e)); // se der erro avisoa
    }
